...
export const stopDesktopSession = async (req, res, next) =>{
  try {
    ...
    const result = await stopDesktopContainer(sessionId, userId, isAdmin);
...
export const extendDesktopSession = async(req, res, next) =>{
  try {
    ...
    // Extend session
    const result = await extendDesktopSessionService(sessionId, additionalSeconds, userId, isAdmin);
