...
export const launchDesktopContainer = async (sessionId, flavor = 'ubuntu', userId) => {
  try {
    ...
    // Wait for container to be ready (desktop containers take longer to start)
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (containerInfo.State.Running) {
          logger.info(`Desktop container ${sessionId} is running and ready`);
          break;
        } else {
          throw new Error(`Desktop container ${sessionId} is not running`);
        }
      } catch (error) {
    ...
    // Additional delay for desktop containers to fully initialize VNC
    logger.info(`Waiting additional time for VNC to initialize...`);
    ...
    // Set up automatic container stop
    const setupAutoCleanup = (session) => {
      const timeUntilExpiry = session.expiresAt.getTime() - Date.now();
      
      if (timeUntilExpiry > 0) {
        session.autoCleanupTimeout = setTimeout(async () => {
          try {
            ...
            if (currentSession && currentSession.status === 'running') {
              ...
              try {
                ...
                await containerToStop.stop({ t: 5 });
                logger.info(`Desktop container auto-stopped: ${sessionId}`);
              } catch (error) {
                if (error.statusCode === 404) {
                  logger.info(`Desktop container ${sessionId} was already stopped or removed`);
                } else {
                  logger.warn(`Failed to stop desktop container ${sessionId}:`, error.message);
                }
              }
              
              activeDesktopSessions.delete(sessionId);
              logger.info(`Desktop session cleaned up: ${sessionId}`);
            }
          } catch (error) {
 ...
 */
export const stopDesktopContainer = async (sessionId, userId) => {
  try {
    ...
    if (!sessionInfo) {
      logger.warn(`Desktop session ${sessionId} not found in active sessions`);
      return false;
    }
    ...
    try {
      ...
      try {
        ...
        if (removeError.statusCode === 404) {
          logger.info(`Desktop container ${sessionId} was already removed (autoRemove)`);
        } else if (removeError.statusCode === 409) {
          logger.info(`Desktop container ${sessionId} removal already in progress`);
        } else {
          logger.warn(`Failed to remove desktop container ${sessionId}:`, removeError.message);
        }
      }
 ...
 */
export const extendDesktopSession = async (sessionId, additionalSeconds = 300, userId) => {
  try {
    logger.info(`Extending desktop session ${sessionId} by ${additionalSeconds} seconds for user ${userId}`);
    ...
    const newExpiryMs = sessionInfo.expiresAt.getTime() + (additionalSeconds * 1000);
    sessionInfo.expiresAt = new Date(newExpiryMs);
    
    // Set up new auto-cleanup timeout
    ...
    if (timeUntilExpiry > 0) {
      sessionInfo.autoCleanupTimeout = setTimeout(async () => {
        try {
          const currentSession = activeDesktopSessions.get(sessionId);
          if (currentSession && currentSession.status === 'running') {
            logger.info(`Auto-stopping expired desktop container after extension: ${sessionId}`);
            ...
            try {
              ...
              logger.info(`Desktop container auto-stopped after extension: ${sessionId}`);
            } catch {
              logger.info(`Desktop container ${sessionId} was already stopped or removed`);
            }
            
            activeDesktopSessions.delete(sessionId);
            logger.info(`Desktop session cleaned up after extension: ${sessionId}`);
          }
        } catch (error) {
          logger.error(`Failed to auto-stop extended desktop container ${sessionId}:`, error.message);
        }
      }, timeUntilExpiry);
    }
    ...
    logger.info(`Desktop session ${sessionId} extension calculation - DEBUG`, {
      sessionId,
      userId,
      additionalSeconds,
      ...
      remainingMinutes,
      now: now.toISOString(),
      calculation: `Previous expiry: ${previousExpiresAt.toISOString()} + Extension: ${additionalSeconds / 60}min = New expiry: ${sessionInfo.expiresAt.toISOString()}`,
      extensionResult: `Extended by ${additionalSeconds} seconds, new total remaining: ${remainingMinutes} minutes`
    });
    ...
    for (const { sessionId, sessionInfo } of expiredSessions) {
      try {
        ...
        try {
          ...
          await container.stop({ t: 10 });
          logger.info(`Stopped expired desktop container: ${sessionId}`);
        } catch (error) {
          if (error.statusCode === 404) {
            logger.info(`Expired desktop container ${sessionId} no longer exists`);
          } else {
            logger.warn(`Failed to stop expired desktop container ${sessionId}:`, error.message);
          }
        }
        
        try {
          await container.remove({ force: true });
          logger.info(`Removed expired desktop container: ${sessionId}`);
        } catch (removeError) {
          if (removeError.statusCode === 404) {
            logger.info(`Expired desktop container ${sessionId} was already removed`);
          } else if (removeError.statusCode === 409) {
            logger.info(`Expired desktop container ${sessionId} removal already in progress`);
          } else {
            logger.warn(`Failed to remove expired desktop container ${sessionId}:`, removeError.message);
          }
        }
